...
//! Avro bytes and decoded back to a typed `(id, text, ts)` tuple.
//!
//! Usage:
//!   cargo run --bin avro_consumer
...
fn handle_message(msg: &IggyMessage) {
    match schemas::avro_deserialize(&msg.payload) {
        Ok((id, text, ts)) => info!(
            "[offset={}] id={} text='{}' ts='{}'",
            msg.header.offset, id, text, ts
        ),
        Err(e) => error!(
            "Failed to decode Avro message at offset {}: {}",
            msg.header.offset, e
        ),
    }
}
