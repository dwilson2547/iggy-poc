    ...
    private static void produceMessages(IggyTcpClient client) {
        ...
        while (true) {
            try {
                ...
                byte[] avroBytes = Schemas.avroSerialize(
                        messageId, "hello from Java SDK Avro producer", ts);

                client.messages().sendMessages(
                        STREAM_ID, TOPIC_ID, partitioning,
                        List.of(messageOfBytes(avroBytes)));

                log.info("Sent Avro message #{}: id={} text='hello from Java SDK Avro producer' ts={}",
                ...
                log.error("Failed to send Avro message #{}", messageId, e);
            }
        }
    }
