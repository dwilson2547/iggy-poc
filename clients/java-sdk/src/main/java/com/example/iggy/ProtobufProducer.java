 ...
 * {@code resources/schemas/event.proto}).  Messages are sent as Protobuf
 * wire-format bytes so that the consumer can decode them with the same schema.
 *
 * <p>Usage: {@code mvn exec:java -Dexec.mainClass=com.example.iggy.ProtobufProducer}
 */
public class ProtobufProducer {
    ...
    private static void produceMessages(IggyTcpClient client) {
        log.info(
                "Producing Protobuf messages to stream='{}' topic='{}' partition={} every {}ms. Press Ctrl+C to stop.",
        ...
        while (true) {
            try {
                ...
                byte[] protoBytes = Schemas.protobufSerialize(
                        messageId, "hello from Java SDK Protobuf producer", ts);

                client.messages().sendMessages(
                        STREAM_ID, TOPIC_ID, partitioning,
                        List.of(messageOfBytes(protoBytes)));

                log.info("Sent Protobuf message #{}: id={} text='hello from Java SDK Protobuf producer' ts={}",
                ...
                log.error("Failed to send Protobuf message #{}", messageId, e);
            }
        }
    }
    ...
    private static void createTopic(IggyTcpClient client) {
        ...
        client.topics().createTopic(
                STREAM_ID, 1L, CompressionAlgorithm.None,
                BigInteger.ZERO, BigInteger.ZERO, empty(), TOPIC_NAME);
        log.info("Topic '{}' was created.", TOPIC_NAME);
    }

    /** Creates a {@link Message} with a raw binary payload (mirrors {@link Message#of(String)}). */
    private static Message messageOfBytes(byte[] payload) {
