...
public class ProtobufProducer {
    ...
    public static void main(String[] args) {
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            logger.error("Fatal error in Protobuf producer", e);
            System.exit(1);
        }
    }
        ...
        while (true) {
            ...
            try {
                ...
                byte[] protoBytes = Schemas.protobufSerialize(
                        messageId, "hello from Java Protobuf producer", ts);
                String payloadB64 = Base64.getEncoder().encodeToString(protoBytes);
                ...
                try (CloseableHttpResponse response = httpClient.execute(request)) {
                    if (response.getCode() >= 200 && response.getCode() < 300) {
                        logger.info(
                                "Sent Protobuf message #{}: id={} text='hello from Java Protobuf producer' ts={}",
                                messageId, messageId, ts);
                    } else {
                        String resp = EntityUtils.toString(response.getEntity());
                        logger.error("Failed to send Protobuf message #{}: HTTP {} - {}",
                                messageId, response.getCode(), resp);
                    }
                }
            } catch (Exception e) {
                logger.error("Failed to send Protobuf message #{}", messageId, e);
            }
