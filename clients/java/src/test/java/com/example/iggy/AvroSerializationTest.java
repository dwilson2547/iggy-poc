...
class AvroSerializationTest {
    ...
    void roundtripProducesOriginalRecord() throws Exception {
        byte[] bytes = Schemas.avroSerialize(1, "hello from Java Avro producer", "2024-01-01T00:00:00Z");
        GenericRecord record = Schemas.avroDeserialize(bytes);

        assertEquals(1, record.get("id"));
        assertEquals("hello from Java Avro producer", record.get("text").toString());
    ...
    @Test
    void decodePayloadBytesHandlesBase64String() {
        String original = "hello";
        String b64 = java.util.Base64.getEncoder().encodeToString(original.getBytes());
        byte[] result = AvroConsumer.decodePayloadBytes(b64);
        assertArrayEquals(original.getBytes(), result);
    }

    @Test
    void decodePayloadBytesHandlesByteList() {
        byte[] original = {104, 101, 108, 108, 111}; // "hello"
        java.util.List<Number> byteList = new java.util.ArrayList<>();
        for (byte b : original) {
            byteList.add(b);
        }
        byte[] result = AvroConsumer.decodePayloadBytes(byteList);
